...
def predict(img, h, w):
    ...
    # Printing the time model run in
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Gathers parameters.')
    ...
    if mode == VID:
        ...
        out = predict(img, h, w)
        out*=255
        cv.imwrite('Stylizedimage.jpg',out)
        # show the image
        if FLAGS.show_original_image:
            cv.imshow('Input Image', img)
        cv.imshow('Stylized image', out/255)
        
        print ('Hit Esc to close!')
        cv.waitKey(0)
